...
export async function getStaticPaths() {
    // Generate the paths for all the dynamic routes
    ...
    return {
        paths,
        fallback: 'blocking'
        //Set to `false` to return a 404 page if the path is not found
    };
}
export async function getStaticProps({ params }) {
    // Fetch the data for the specific product based on the `id` parameter
    const inventory =  await stripe.products.list({
